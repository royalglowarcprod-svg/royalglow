...
export default function RegisterPage() {
  ...
  // Reuse exact same CSS as login page — just copy the css const from login
  return (
    <>
      <style>{css}</style>
      <div className="ap">
        <div className="ac">
          ...
          {success ? (
            <div style={{textAlign:"center",padding:"20px 0"}}>
              ...
              <h2 style={{color:"#e8eaf0",fontWeight:600,marginBottom:"8px"}}>Check your email</h2>
              <p style={{color:"#545a66",fontSize:"0.85rem"}}>We sent a confirmation link to <strong style={{color:"#7c9e87"}}>{email}</strong></p>
              <button className="asb" style={{marginTop:"24px"}} onClick={() => router.push("/login")}>Back to Sign In</button>
            </div>
          ) : (
            <>
              <h1 className="at">Create account</h1>
              <p className="as">Join CrashCart today</p>
              <div className="ao">
              ...
              <div className="af">
                ...
                <div className="afd"><label className="afl">Full Name</label><input className="afi" type="text" placeholder="John Doe" value={name} onChange={e => setName(e.target.value)}/></div>
                <div className="afd"><label className="afl">Email</label><input className="afi" type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)}/></div>
                <div className="afd"><label className="afl">Password</label><input className="afi" type="password" placeholder="Min. 6 characters" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key==="Enter"&&register()}/></div>
                <button className="asb" onClick={register} disabled={loading}>{loading?<span className="asp"/>:"Create Account"}</button>
              </div>
              <p className="asw">Already have an account? <button className="asl" onClick={() => router.push("/login")}>Sign in</button></p>
            </>
          )}
        </div>
      </div>
